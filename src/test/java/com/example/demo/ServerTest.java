...
class ServerTest extends AbstractServerTest {
    ...
    void testCloseCommand() throws Exception {
        ...
        assertEquals(closed, sendMessage("Applejack CLOSE_TASK FindApples"),
                "Таска должна закрытаться в штатном режиме");
        endMessage();
        ...
        endMessage();
        assertEquals(error, sendMessage("Applejack CLOSE_TASK SomeRandomText"),
                "Нельзя закрывать несуществующие таски");
    }
    ...
    void testDeleteCommand() throws Exception {
        ...
        endMessage();
        assertEquals(error, sendMessage("PinkyPie DELETE_TASK SomeRandomText"),
                "Нельзя закрывать несуществующие таски");
    }
    ...
    void testReopenCommand() throws Exception {
        ...
        endMessage();
        assertEquals(error, sendMessage("CyrodiilChamp REOPEN_TASK SomeRandomText"),
                "Нельзя закрывать несуществующие таски");
    }
    ...
    void testList() throws Exception {
        ...
        endMessage();
        assertEquals("TASKS [Bastards]", sendMessage("Cartman LIST_TASK Kenny"),
                "не должно быть запрета просмотра чужих записей");
        endMessage();
        sendMessage("Cartman CLOSE_TASK goToKFC");
        endMessage();
        sendMessage("Cartman DELETE_TASK goToKFC");
        endMessage();
        assertEquals("TASKS [MakeWorldBetter]", sendMessage("Cartman LIST_TASK Cartman"),
                "Удаленные записи не должны быть показаны");
    }

    @Test
    @DisplayName("Тест формата ввода")
    void testFormat() throws Exception {
        String wrongFormat = "WRONG_FORMAT";
